...
const Column: React.FC<typeProps> = ({id, tables, setColumnWrap, setIds}) => {
    const nameRef = useRef<HTMLInputElement>(null);
    const keyRef = useRef<HTMLInputElement>(null);
    const name = useRef<HTMLInputElement>(null);                   
    const key = useRef<HTMLInputElement>(null);
    const stored = localStorage.getItem("id") ? JSON.parse( localStorage.getItem("id")!) : "" ;
    const [ids, setId] = useState<string >(stored);
    const columnStored = localStorage.getItem("columns")? JSON.parse(localStorage.getItem("columns")!):[]
    const [columns, setColumns] = useState<ColumnProps[]>(columnStored);

    function validate(name: HTMLInputElement | null, key: HTMLInputElement | null) {
        if (!name?.value.trim()) {
    ...
    return (
        ...
        <div className="modal-dialog">
            ...
            <div className="modal-header">
                <h1 className="modal-title fs-5" id="exampleModalLabel">Edit message</h1>
            ...
            <div className="modal-body">
                <form>
                    ...
                    <div className="mb-3">
                        <label htmlFor="recipient-name" className="col-form-label">Key:</label>
                        <input ref={key} type="password" className="form-control" id="recipient-name"/>
                    </div>
                </form>
            </div>
            <div className="modal-footer">
                <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button onClick={handleEdit} type="button" className="btn btn-primary" data-bs-dismiss="modal">Save message</button>
            </div>
            </div>
        </div>
        </div>
            ...
            <form onSubmit={handleSubmit} name="form" className="column">
                <img onClick={handleDelete} src={menu} width={32} height={32} alt="menu icon" />
                <div className="input-column">
                    <label htmlFor="ustun">Ustun nomi</label>
                    <input ref={nameRef} id="ustun" type="text" placeholder="Ustun nomi"/>
                </div>
                <div className="input-key">
                    <label htmlFor="key">Key</label>
                    <input ref={keyRef} id="key" type="password" placeholder="Key"/>
                </div>
                        ...
                        tables && tables.map((el, index) => (
                            <tr key={el.id}>
                                ...
                                <td className="d-flex justify-content-center m-0 p-0">
                                    <span data-bs-toggle="modal" data-bs-target= "#exampleModal" data-bs-whatever="@fat" onClick={() => {Edited(el)}} className="text-danger">edit</span>
                                    <span onClick={() => {Deleted(el.id)}} className="text-success">trash</span>
                                </td>
                            </tr>
                        ))
                    }
